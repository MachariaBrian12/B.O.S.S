...
export default function InputPage() {
  ...
  useEffect(()=>{
    ...
    api.getToday(token).then(d=>{
      if(d.entry){
        setExisting(d.entry);
        setForm({ sales:String(d.entry.sales), expenses:String(d.entry.expenses), notes:d.entry.notes||"" });
      }
    }).catch(()=>{});
  },[token]);
  ...
  const profit = form.sales && form.expenses ? parseFloat(form.sales||"0") - parseFloat(form.expenses||"0") : null;
      ...
      <motion.div initial={{opacity:0,y:18}} animate={{opacity:1,y:0}} transition={{duration:.5}}
        ...
        <div style={{marginBottom:28}}>
          <Link href="/dashboard" style={{fontSize:12,color:"var(--t3)",textDecoration:"none",
            display:"inline-flex",alignItems:"center",gap:6,marginBottom:20,transition:"color .2s"}}
            onMouseEnter={e=>(e.currentTarget.style.color="var(--t1)")}
            onMouseLeave={e=>(e.currentTarget.style.color="var(--t3)")}>
            ← Dashboard
          </Link>
          <div style={{fontSize:10,color:"var(--cyan)",letterSpacing:".12em",textTransform:"uppercase",marginBottom:6}}>
            {existing ? "Edit Today's Entry" : "Daily Entry"}
          </div>
          <h1 style={{fontFamily:"'Syne',sans-serif",fontSize:22,fontWeight:800,color:"#F1F5F9",letterSpacing:"-.02em"}}>
            {new Date().toLocaleDateString("en-KE",{weekday:"long",month:"long",day:"numeric"})}
          </h1>
          {existing && (
            <div style={{fontSize:11,color:"var(--amber)",marginTop:4}}>✏️ Editing existing entry</div>
          )}
        </div>
        ...
        {success && (
          <div style={{background:"rgba(16,185,129,.1)",border:"1px solid rgba(16,185,129,.25)",
            borderRadius:10,padding:"12px 16px",marginBottom:20,fontSize:13,color:"#10B981",textAlign:"center"}}>
            ✓ Saved! Redirecting...
          </div>
        )}
        ...
        <form onSubmit={submit}>
          ...
          {profit !== null && (
            <motion.div initial={{opacity:0}} animate={{opacity:1}}
              style={{marginBottom:16,padding:"12px 16px",
                background:profit>=0?"rgba(16,185,129,.07)":"rgba(239,68,68,.07)",
                border:`1px solid ${profit>=0?"rgba(16,185,129,.15)":"rgba(239,68,68,.15)"}`,borderRadius:10}}>
              <div style={{fontSize:10,color:profit>=0?"#10B981":"#EF4444",letterSpacing:".08em",textTransform:"uppercase",marginBottom:4}}>Net Profit</div>
              <div style={{fontFamily:"'Syne',sans-serif",fontSize:22,fontWeight:700,color:profit>=0?"#10B981":"#EF4444"}}>
          ...
          <div style={{display:"flex",gap:10}}>
            <button className="btn btn-primary" type="submit"
              ...
              {loading?"Saving...":success?"Saved ✓":existing?"Update Entry →":"Save Entry →"}
            </button>
            {existing && (
              <button type="button" onClick={()=>setShowDeleteConfirm(true)}
                style={{padding:"14px 18px",borderRadius:10,border:"1px solid rgba(239,68,68,.3)",
                  background:"rgba(239,68,68,.08)",color:"#EF4444",fontSize:14,cursor:"pointer"}}>
                🗑
              </button>
            )}
          </div>
        ...
        <AnimatePresence>
          {showDeleteConfirm && (
            <motion.div initial={{opacity:0,y:10}} animate={{opacity:1,y:0}} exit={{opacity:0,y:10}}
              style={{marginTop:16,padding:"16px",background:"rgba(239,68,68,.08)",
                border:"1px solid rgba(239,68,68,.2)",borderRadius:12}}>
              <p style={{fontSize:13,color:"#EF4444",marginBottom:12}}>Delete today's entry? This cannot be undone.</p>
