...
export default function ProfilePage() {
  ...
  const { user, token, setUser } = useBusinessStore(s => s);
    ...
    <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",padding:"24px",position:"relative"}}>
      ...
      <motion.div initial={{opacity:0,y:18}} animate={{opacity:1,y:0}} transition={{duration:.5}}
        ...
        <Link href="/dashboard" style={{fontSize:12,color:"var(--t3)",textDecoration:"none",
          display:"inline-flex",alignItems:"center",gap:6,marginBottom:24,transition:"color .2s"}}
        ...
        <div style={{display:"flex",alignItems:"center",gap:16,marginBottom:32}}>
          ...
          <div>
            <div style={{fontFamily:"'Syne',sans-serif",fontSize:18,fontWeight:700,color:"#F1F5F9"}}>{user?.name}</div>
        ...
        {success && (
          <div style={{background:"rgba(16,185,129,.1)",border:"1px solid rgba(16,185,129,.25)",
            borderRadius:10,padding:"12px 16px",marginBottom:20,fontSize:13,color:"#10B981"}}>
            ✓ {success}
          </div>
        )}
        ...
        <form onSubmit={submit}>
          <div style={{marginBottom:14}}>
            ...
            <input className="input" type="text" value={form.name} onChange={update("name")} required/>
          </div>
          <div style={{marginBottom:24}}>
            <label className="label">Business Name</label>
            <input className="input" type="text" value={form.business} onChange={update("business")} required/>
          </div>

          <div style={{borderTop:"1px solid rgba(255,255,255,.06)",paddingTop:20,marginBottom:14}}>
            <div style={{fontSize:10,color:"var(--t3)",letterSpacing:".1em",textTransform:"uppercase",marginBottom:14}}>
