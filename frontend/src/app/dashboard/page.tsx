        ...
        <motion.aside initial={{x:-220}} animate={{x:0}}
          ...
          <div style={{borderTop:"1px solid rgba(255,255,255,.05)",paddingTop:14}}>
            <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10,padding:"0 2px"}}>
              ...
              <div>
                <div style={{fontSize:11,color:"#F1F5F9",fontWeight:500}}>{user?.name?.split(" ")[0]}</div>
            ...
            <button onClick={handleLogout} style={{
              ...
              onMouseEnter={e=>{(e.currentTarget as HTMLElement).style.color="#EF4444";(e.currentTarget as HTMLElement).style.borderColor="rgba(239,68,68,.2)";}}
              onMouseLeave={e=>{(e.currentTarget as HTMLElement).style.color="#334155";(e.currentTarget as HTMLElement).style.borderColor="rgba(255,255,255,.05)";}}>
              Sign out
            </button>
          </div>
        </motion.aside>
        ...
        <main style={{flex:1,overflowY:"auto",padding:"32px 32px 60px"}}>
          ...
          <motion.div variants={fadeUp} initial="hidden" animate="show"
            ...
            <Link href="/input" style={{
              ...
              onMouseEnter={e=>{(e.currentTarget as HTMLElement).style.transform="translateY(-2px)";(e.currentTarget as HTMLElement).style.boxShadow="0 14px 40px rgba(59,130,246,.42)";}}
              onMouseLeave={e=>{(e.currentTarget as HTMLElement).style.transform="none";(e.currentTarget as HTMLElement).style.boxShadow="0 6px 24px rgba(59,130,246,.28)";}}>
              + Add Entry
            </Link>
          </motion.div>
          ...
          {!ins?.hasData && (
            <motion.div variants={fadeUp} initial="hidden" animate="show" exit={{opacity:0}}
              style={{background:"rgba(255,255,255,.02)",border:"1px solid rgba(255,255,255,.06)",
                borderRadius:18,padding:"56px 40px",textAlign:"center",marginBottom:28}}>
              <div style={{fontSize:44,marginBottom:14}}>📊</div>
              <h2 style={{fontFamily:"'Syne',sans-serif",fontSize:20,fontWeight:700,marginBottom:10,color:"#F1F5F9"}}>
                No data yet for today
              </h2>
              <p style={{color:"#94A3B8",fontSize:14,marginBottom:26,fontWeight:300,maxWidth:380,margin:"0 auto 26px"}}>
            ...
            <motion.div variants={stagger} initial="hidden" animate="show">
                ...
                {[
                  ...
                  <motion.div key={c.label} variants={fadeUp}
                    ...
                    <div style={{position:"absolute",inset:0,
                      ...
                      pointerEvents:"none"}}/>
                    <div style={{fontSize:9,color:"#334155",letterSpacing:".1em",textTransform:"uppercase",marginBottom:9}}>{c.label}</div>
                    <div style={{fontFamily:"'Syne',sans-serif",fontSize:22,fontWeight:700,color:c.color,
                      letterSpacing:"-.02em",marginBottom:4}}>{c.value}</div>
              ...
              <motion.div variants={stagger}
                ...
                <motion.div variants={fadeUp}
                  ...
                  <div style={{fontSize:9,color:"#334155",letterSpacing:".1em",textTransform:"uppercase",marginBottom:18}}>
                    Business Health
                  </div>
              ...
              <motion.div variants={fadeUp}
                ...
                {/* tab bar */}
                <div style={{display:"flex",borderBottom:"1px solid rgba(255,255,255,.055)",padding:"0 20px"}}>
                  {([
                    {key:"feed",    label:"Intelligence Feed", count:feed.length,    color:"#8B5CF6"},
                    {key:"alerts",  label:"Alerts",            count:alerts.length,  color:"#EF4444"},
                    {key:"signals", label:"Predictive Signals",count:signals.length, color:"#06B6D4"},
                  ] as const).map(t=>(
                    <button key={t.key} onClick={()=>setTab(t.key)}
                      style={{
                        padding:"14px 16px",fontSize:11,fontWeight:500,cursor:"pointer",
                      ...
                      {t.label}
                      {t.count>0 && (
                        <span style={{padding:"1px 6px",borderRadius:100,fontSize:9,fontWeight:600,
                ...
                <div style={{padding:"16px 20px",minHeight:200,maxHeight:340,overflowY:"auto"}}>
                          ...
                          return (
                            <motion.div key={item.id}
                              ...
                              <div style={{width:28,height:28,borderRadius:8,flexShrink:0,
                                ...
                                fontSize:12,color:typeColor,fontWeight:700}}>
                                {item.icon}
                              </div>
                              <div style={{flex:1}}>
                                <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:4}}>
                                  ...
                                  <span style={{fontSize:9,color:"#334155",letterSpacing:".06em",
                                    ...
                                    background:"rgba(255,255,255,.04)"}}>{item.time}</span>
                                </div>
                            ...
                            <motion.div key={i}
                              ...
                              <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}>
                                <span style={{fontSize:9,fontWeight:700,letterSpacing:".1em",textTransform:"uppercase",
                                  color:c,padding:"2px 8px",borderRadius:100,background:`${c}18`}}>
                                  {alert.level}
                                </span>
                                <span style={{fontSize:12,fontWeight:600,color:"#F1F5F9"}}>{alert.title}</span>
                              </div>
                              <p style={{fontSize:12,color:"#64748B",lineHeight:1.55,marginBottom:8,fontWeight:300}}>
                                {alert.body}
                              </p>
                          ...
                          return (
                            <motion.div key={sig.id}
                              ...
                              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:6}}>
                                <div style={{display:"flex",alignItems:"center",gap:8}}>
                                  <span style={{fontSize:13,color:c}}>
                                    {sig.direction==="up"?"↑":sig.direction==="down"?"↓":"→"}
                                  </span>
                                  <span style={{fontSize:12,fontWeight:600,color:"#F1F5F9"}}>{sig.title}</span>
                                </div>
                              ...
                              <p style={{fontSize:12,color:"#64748B",lineHeight:1.55,marginBottom:6,fontWeight:300}}>
                                {sig.body}
                              </p>
              ...
              {ins.weekStats && (
                <motion.div variants={fadeUp}
                  ...
                  <div style={{fontSize:9,color:"#334155",letterSpacing:".1em",textTransform:"uppercase",marginBottom:14}}>
                    Weekly Summary
                  </div>
                  <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:14}}>
                    {[
                      ...
                      <div key={s.label}
                        style={{borderLeft:`2px solid ${s.color}35`,paddingLeft:10}}>
                        <div style={{fontSize:9,color:"#334155",letterSpacing:".07em",textTransform:"uppercase",marginBottom:5}}>
                          {s.label}
                        </div>
                        <div style={{fontFamily:"'Syne',sans-serif",fontSize:15,fontWeight:700,color:s.color}}>
                          {s.value}
                        </div>
                      </div>
                    ))}
