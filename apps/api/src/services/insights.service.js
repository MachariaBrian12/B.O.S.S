...
// No currency formatting on backend — frontend handles display
const num = (n) => Number(n || 0).toLocaleString();
const pct = (a, b) => (b > 0 ? (((a - b) / b) * 100).toFixed(1) : null);

const getStreak = async (userId) => {
  const { rows } = await pool.query(
    'SELECT date FROM business_entries WHERE user_id=$1 ORDER BY date DESC',
    [userId],
  );
...
const generateFeed = (today, yesterday, history) => {
  ...
  const margin =
    today.sales > 0 ? ((profit / today.sales) * 100).toFixed(1) : 0;
  ...
  if (yesterday) {
    ...
    if (salesDiff !== null) {
      ...
      feed.push({
        id: 'sales-vs-yday',
        type: up ? 'growth' : 'warning',
        icon: up ? '↑' : '↓',
        title: up ? 'Revenue momentum' : 'Revenue softened',
        body: `Sales ${up ? 'up' : 'down'} ${Math.abs(parseFloat(salesDiff))}% vs yesterday.`,
  ...
  if (Number(margin) >= 45) {
    feed.push({
      ...
      body: `${margin}% profit margin — above the healthy 30% threshold.`,
      time: 'Today',
      priority: 4,
    });
    ...
    feed.push({
      ...
      body: `Profit margin at ${margin}% — expenses consuming ${(expRatio * 100).toFixed(0)}% of revenue.`,
      time: 'Today',
      priority: 0,
    });
  }
  ...
  if (history.length >= 3) {
    const avgSales =
      history.slice(0, 7).reduce((a, r) => a + Number(r.sales), 0) /
    ...
    if (diff !== null && Math.abs(parseFloat(diff)) > 15) {
      ...
      feed.push({
        id: 'vs-weekly-avg',
        type: up ? 'opportunity' : 'warning',
        icon: up ? '⬆' : '⬇',
        title: up ? 'Above weekly average' : 'Below weekly average',
        body: `Today's sales ${Math.abs(parseFloat(diff))}% ${up ? 'above' : 'below'} your 7-day average.`,
        time: '7-day average',
        priority: up ? 3 : 2,
      });
    }
  ...
  if (expRatio > 0.7) {
    feed.push({
      ...
      body: `Expenses at ${(expRatio * 100).toFixed(0)}% of revenue. Sustainable max is 60%.`,
      time: 'Today',
      priority: 1,
    });
  }
...
const generateSignals = (today, history) => {
  ...
  if (salesTrend < -0.1 * recent3[2].sales) {
    signals.push({
      ...
      body: 'Revenue has declined over the last 3 days. Consider a targeted promotion.',
    ...
    signals.push({
      ...
      body: 'Sales have grown consistently. Ensure inventory can meet continued demand.',
      timeframe: 'Next 1–2 days',
    });
  }
  if (expTrend > 0.15 * recent3[2].expenses) {
    signals.push({
      ...
      body: 'Expenses rising consistently. Cash flow may tighten next week.',
  ...
  if (!signals.length) {
    signals.push({
      ...
      body: 'No anomalies detected. Continue monitoring daily performance.',
      timeframe: 'Near term',
    });
  }
  return signals;
...
const generateAlerts = (today, yesterday, history) => {
  ...
  if (profit < 0)
    alerts.push({
      ...
      body: `Expenses exceed sales by ${Math.abs(profit).toLocaleString()}.`,
      action: 'Review and cut non-essential expenses today.',
    });
  if (expRatio > 0.75 && profit >= 0)
    alerts.push({
      level: 'warning',
      title: 'Expenses too high',
      body: `${(expRatio * 100).toFixed(0)}% of revenue going to expenses.`,
      action: 'Identify top 2 expenses and reduce them.',
    });
  if (yesterday && today.sales < yesterday.sales * 0.65)
    alerts.push({
      level: 'warning',
      title: 'Significant sales drop',
      body: `Revenue fell 35%+ vs yesterday.`,
      action: 'Check pricing, visibility, and stock.',
    });
  if (margin >= 40 && today.sales > 0)
    alerts.push({
      level: 'opportunity',
      title: 'Strong margin window',
      body: `${margin.toFixed(1)}% margin today. Prime time to reinvest.`,
...
const runInsights = async (userId) => {
  ...
  if (!today) {
    return {
      hasData: false,
      profitTrend: 'neutral',
      topProduct: 'No products recorded yet',
      summary: 'No data entered yet for today.',
      warning: null,
      recommendation:
        "Enter today's sales and expenses to get your first insight.",
      score: 50,
      feed: [],
      signals: generateSignals(null, history),
      alerts: [],
      streak,
      history: [],
    };
  ...
  const marginPct =
    today.sales > 0 ? ((profit / Number(today.sales)) * 100).toFixed(1) : 0;
  const expRatio = today.sales > 0 ? today.expenses / today.sales : 0;
  ...
  if (yesterday) {
    ...
    if (profit > prevProfit) {
      ...
      const p =
        prevProfit > 0
          ? (((prevProfit - profit) / prevProfit) * 100).toFixed(1)
          : 100;
      profitTrend = `-${p}%`;
    } else {
      profitTrend = '0%';
  ...
  let warning = null;
  if (profit < 0) {
    warning = 'Operating at a loss. Expenses exceed sales.';
    score -= 25;
  } else if (expRatio > 0.75) {
    warning = 'Expenses above 75% of sales — margins critically thin.';
    score -= 18;
  } else if (expRatio > 0.5) {
    warning = 'Expenses above 50% of sales.';
    score -= 8;
  }

  let recommendation =
    'Stay consistent. Daily data builds your intelligence model.';
  ...
  else if (Number(marginPct) >= 40)
    recommendation = 'Excellent margins. Good time to invest in marketing.';

  const { rows: topP } = await pool.query(
    'SELECT name FROM products WHERE user_id=$1 ORDER BY revenue DESC LIMIT 1',
    [userId],
  );
  ...
  // Summary uses raw numbers — frontend formats with user's currency
  return {
    hasData: true,
    today: {
      sales: Number(today.sales),
      expenses: Number(today.expenses),
      profit,
    ...
    yesterday: yesterday
      ? {
          sales: Number(yesterday.sales),
          expenses: Number(yesterday.expenses),
          profit: Number(yesterday.sales) - Number(yesterday.expenses),
        }
      : null,
    profitTrend,
    topProduct,
    summary: `${num(today.sales)} in sales · ${num(today.expenses)} in expenses · ${num(profit)} profit (${marginPct}% margin).`,
    warning,
    recommendation,
    ...
    history: history.map((e) => ({
      ...
      profit: Number(e.sales) - Number(e.expenses),
    })),
    feed,
    signals,
    alerts,
